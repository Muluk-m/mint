...
import { Tabs, Radio, Checkbox, Row, Col, InputNumber, Space, Input, message } from 'antd';
import getI18n from './utils/zh';
import PropTypes from 'prop-types';
import { cronValidate } from './utils/validate';
import './index.scss';
...
const options = Array.from({ length: 60 }).map((_, i) => String(i));
const hourOptions = Array.from({ length: 24 }).map((_, i) => String(i));
...
class CRON extends React.Component {
  static propTypes = {
    ...
    defalutCron: PropTypes.string
  };

  constructor(props) {
    super(props);
    const { value, type = ['minute', 'hour', 'day', 'month'], defalutCron = '0 0 0 * * ?' } = props;
    const values = value && value !== '' && cronValidate(value) ? value : defalutCron;
  ...
  onWeekCheckChange = (checkedValues) => {
    const weekChecked = checkedValues.length > 0 ? checkedValues.join(',') : '1';
  ...
  render() {
    ...
    try {
      // 去掉年份参数
      const e = `${secondVal} ${minVal} ${hourVal} ${dayOfMonVal} ${MonVal} ${dayOfWekVal}`;
      const format = 'yyyy-MM-dd hh:mm:ss';
      const now = Date.now();
      const options = {
        currentDate: dateFormat(now, format)
      };
      const iter = cronParser.parseExpression(e, options);
      const result = [];
      const hasWeek = !['*', '?'].includes(dayOfWekVal);
      for (let i = 0; i < 5; i++) {
        // cronParser解析 周域 1为周一, 而我们的标准 1为周日,故减去一天
        const t = hasWeek
          ? moment(new Date(iter.next())).subtract(1, 'days')
          : moment(new Date(iter.next()));
        result.push(t.format('YYYY-MM-DD HH:mm:ss'));
      }
      this.preTimeList = result.length > 0 ? result.join('\n') : '无执行时间';
    } catch (e) {
      message.error('cron表达式错误');
      console.log(e);
    }

    const { style = { width: 576 }, className } = this.props;
    // const i18n = getI18n(lang); // 国际化
    const i18n = getI18n; // 国际化
    const Cls = classnames('cron', className);
    return (
      <div className={Cls} style={{ ...style, padding: '0 10px' }}>
        <div>注意:Cron表达式每个时间域是相互独立的,需要单独设置!</div>
                    ...
                    <Row>
                      {
                        ...
                        weekOptions.map((dayName, i) => (
                          <Col key={dayName} span={3}>
                            <Checkbox disabled={weekRadiochecked !== 6} value={i + 1 + ''}>
                              {dayName}
                            </Checkbox>
                          </Col>
                        ))
        ...
        <div style={{ margin: '-10px 0 20px' }}>
          <Space>
            <span>表达式:</span>
            <span
              style={{ border: '1px solid #ddd', padding: '5px 10px', borderRadius: 3 }}
            >{`${secondVal} ${minVal} ${hourVal} ${dayOfMonVal} ${MonVal} ${dayOfWekVal}`}</span>
          </Space>
        </div>
        <div>
          未来五次调度时间：
